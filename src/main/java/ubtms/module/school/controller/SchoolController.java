...
public class SchoolController {
    ...
    @RequestMapping("/schoolMsgPage")
    public String schoolMsgPage(HttpServletRequest request){
        String schoolName = request.getSession().getAttribute("loginSchool").toString();
        return "school/schoolMsg";
    }
    ...
    public String viewAndEditSchool(HttpServletRequest request, Model model) {
        String schIdStr = request.getParameter("schId");
        int schId = 0;
        School school = null;
        if (schIdStr == null) {
            school = (School) request.getSession().getAttribute("loginSchool");
        }else {
            schId = Integer.parseInt(schIdStr);
            school = schoolService.selectOne(schId);
        }
        school.getSchLogo();
